...
export function OperatingActorsModal({
  actors,
  open,
  onOpenChange,
  onViewGap,
}: OperatingActorsModalProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Organizaciones actualmente desplegadas en terreno
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4 pr-4">
            {actors.length === 0 ? (
              <p className="text-muted-foreground text-center py-8">
                No hay actores operando actualmente
              </p>
            ) : (
        ...
        <Alert variant="default" className="mt-4 bg-muted/50">
          <AlertDescription className="text-sm text-muted-foreground">
            La presencia de actores no implica brechas contenidas.
          </AlertDescription>
        </Alert>
      </DialogContent>
    </Dialog>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <Badge
              variant="outline"
              className="text-xs text-coverage border-coverage/50"
            >
      ...
      <div className="text-sm space-y-1">
        ...
        {lastConfirmed && (
          <p className="text-muted-foreground text-xs">
            Última confirmación: {lastConfirmed}
          </p>
        )}
      </div>
